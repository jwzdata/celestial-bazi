...
// Database setup
const db = new sqlite3.Database('./bazi.db');
db.serialize(() => {
...
app.post('/api/register', (req, res) => {
  ...
  db.get('SELECT id FROM users WHERE username = ?', [username], (err, row) => {
    ...
    db.run('INSERT INTO users (username, password, invite_code, invited_by) VALUES (?, ?, ?, ?)',
      [username, hash, inviteCode, ref || null],
      function(err) {
...
app.get('/api/user', authenticateToken, (req, res) => {
  db.get('SELECT id, username, invite_code, vip_expire_time, balance FROM users WHERE id = ?', [req.user.id], (err, user) => {
...
app.post('/api/pay/create', authenticateToken, (req, res) => {
  ...
  db.run('INSERT INTO orders (id, user_id, amount, payment_method) VALUES (?, ?, ?, ?)',
    [orderId, req.user.id, amount, method],
    (err) => {
...
app.post('/api/pay/mock-success', authenticateToken, (req, res) => {
  ...
  db.get('SELECT * FROM orders WHERE id = ? AND status = "pending"', [orderId], (err, order) => {
    ...
    db.get('SELECT vip_expire_time, invited_by FROM users WHERE id = ?', [order.user_id], (err, user) => {
      ...
      db.run('UPDATE users SET vip_expire_time = ? WHERE id = ?', [newExpire, order.user_id]);

      // 处理分销佣金 (30% 佣金 = 3元)
      if (user.invited_by) {
        db.run('UPDATE users SET balance = balance + 3 WHERE invite_code = ?', [user.invited_by]);
      }
